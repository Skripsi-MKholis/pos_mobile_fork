...
class _TransactionHistoryScreenState
        ...
        slivers: [
          ...
          historyAsync.when(
            data: (state) {
              ...
              // Optimization: Hoist invariant search query and time evaluations outside the loop
              // to prevent redundant computations and memory allocations on every iteration.
              ...
              final filtered = transactions.where((tx) {
                final matchesSearch =
                    tx['id'].toString().toLowerCase().contains(queryLower) ||
  ...
  Widget _buildSummaryCards(
    ...
    return historyAsync.when(
      data: (state) {
        // Optimization: Hoist invariant time evaluations outside the loop
        // to prevent redundant computations on every iteration.
        final DateTime now = DateTime.now();
        final DateTime yesterday = now.subtract(const Duration(days: 1));
