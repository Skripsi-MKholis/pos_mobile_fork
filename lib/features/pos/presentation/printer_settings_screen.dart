...
class _PrinterSettingsScreenState extends ConsumerState<PrinterSettingsScreen> {
    ...
    try {
      ...
      if (mounted) {
        ShadToaster.of(context).show(
          ShadToast.destructive(description: Text('Error: $e')),
        );
      }
    } finally {
  ...
  Widget _buildStatusHeader(BluetoothDevice? connectedDevice, ShadThemeData theme) {
    ...
    return Container(
      ...
      color: isConnected ? theme.colorScheme.primary.withValues(alpha: 0.1) : theme.colorScheme.muted,
      child: Column(
        children: [
          Icon(
            isConnected ? TablerIcons.printer : TablerIcons.printer_off,
            size: 56,
            color: isConnected ? theme.colorScheme.primary : theme.colorScheme.mutedForeground,
          ),
          ...
          if (isConnected) ...[
            ...
            ShadButton.destructive(
              size: ShadButtonSize.sm,
              onPressed: () => ref.read(printerNotifierProvider.notifier).disconnect(),
